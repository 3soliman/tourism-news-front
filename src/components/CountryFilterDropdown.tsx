  ...
  return (
    <li ref={rootRef} className="relative">
      <button
        ...
        className={`flex w-full items-center justify-between gap-2 rounded-sm px-3 py-3 text-sm font-black transition lg:my-2 lg:inline-flex lg:justify-start lg:py-2.5 ${
          open || activeSlug
            ? "bg-accent text-[#244958]"
            : "text-white hover:bg-white/12 hover:text-accent"
        }`}
      >
      ...
      {open ? (
        <div
          ...
          className="absolute right-0 top-full z-60 mt-0 w-72 overflow-hidden rounded-b-sm border border-border bg-white shadow-xl lg:mt-0"
        >
          ...
          <div className="max-h-72 overflow-y-auto p-2">
            <Link
              ...
              onClick={closeMenu}
              className={`flex items-center gap-3 rounded-sm px-3 py-2.5 text-sm font-bold transition ${
                !activeSlug
                  ? "bg-primary text-white"
                  : "text-text-dark hover:bg-page-bg hover:text-primary"
              }`}
            >
              <Globe2 className="h-4 w-4 shrink-0" aria-hidden="true" />
              <span>كل الدول</span>
